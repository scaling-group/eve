...
solver_root="${EVE_SOLVER_ROOT:-$workspace_root/solver}"
...
support_dir="$(cd "${script_dir}/support" && pwd)"
source "${support_dir}/helpers.sh"
...
cleanup() {
  ...
  if (( exit_code != 0 )) && (( OUTCOME_IS_CANDIDATE_FAILURE == 1 )); then
    if write_failure_outputs "${ERROR_MESSAGE:-evaluation.sh failed with exit code ${exit_code}}"; then
      final_exit_code=0
    fi
  fi
...
prepare_stage_bundle() {
  ...
  python3 - \
    ...
    "${solver_root}" \
    "${META_JSON_PATH}" \
    "${script_dir}/evaluation_orchestrator.sh" \
    "${support_dir}/orchestrator.py" \
    "${support_dir}/status_watcher.py" \
    "${support_dir}/launch_watched_job.sh" \
    "${EDITABLE_FILES[@]}" <<'PY'
import shutil
import sys
from pathlib import Path
...
for relative_path in "${EDITABLE_FOLDERS[@]:-}"; do
  ...
  if [[ ! -d "${solver_root}/${relative_path}" ]]; then
    mark_transport_halt "Missing editable folder: ${solver_root}/${relative_path}"
    exit 1
  fi
done

for relative_path in "${EDITABLE_FILES[@]}"; do
  if [[ ! -f "${solver_root}/${relative_path}" ]]; then
    mark_transport_halt "Missing editable file: ${solver_root}/${relative_path}"
    exit 1
  fi
done
...
JOB_ID=""
if ! JOB_ID="$(submit_watched_pbs_job "launch_watched_job.sh" "evaluation_orchestrator.sh" "evaluation orchestrator")"; then
  mark_transport_halt "Failed to submit evaluation orchestrator PBS job"
  exit 1
fi

pbs_wait_rc=0
if wait_for_status_file "${JOB_ID}" "${STAGING_DIR}/status.json" "${STATUS_POLL_INTERVAL_SECONDS}" "evaluation orchestrator"; then
  pbs_wait_rc=0
else
  pbs_wait_rc=$?
fi
...
if [[ ! -s "${RESULT_JSON_PATH}" ]]; then
  emit_pbs_fetch_diagnostics "${JOB_ID}" "evaluation_orchestrator" || true
  case "${pbs_wait_rc}" in
