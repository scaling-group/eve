"""Import Eve populations from previous run directories.

`import_from` config accepts a list. Each item may be either:
...
2. a mapping with optional id filters, example:
   ...
   If `solver_ids` is omitted, import every solver from that source.
   If `optimizer_ids` is omitted, import every optimizer from that source.
   Set `import_solvers: false` or `import_optimizers: false` to skip one
   side of the population while importing the other.
...
- `solver_ids` omitted means import every solver from that source.
- `optimizer_ids` omitted means import every optimizer from that source.
- multiple `import_from` entries are additive and are imported in order.
"""
...
class PopulationImportResult:
    """Summary for one imported Eve population."""
...
@dataclass(frozen=True)
class ImportResult:
    """Summary for one import operation."""

    source_run_root: Path
    solver: PopulationImportResult
    optimizer: PopulationImportResult


@dataclass(frozen=True)
class ImportSpec:
    """One import source plus optional entry-id filters.
    ...
    - `import_solvers`: import solver entries from this source
    - `import_optimizers`: import optimizer entries from this source

    Omitted or empty `solver_ids` / `optimizer_ids` means "import all entries
    ...
    import_solvers: bool = True
    import_optimizers: bool = True


def import_populations_from_run(
    ...
    *,
    ...
    """Import solver and optimizer populations from a previous Eve run.

    `source_path` may point at the run root itself or any nested directory inside it.
    """

    source_run_root = resolve_run_root(source_path)
    solver_entries = (
        _load_population_entries(
    ...
    return ImportResult(
        ...
        solver=PopulationImportResult(entries_imported=len(solver_entries)),
        optimizer=PopulationImportResult(entries_imported=len(optimizer_entries)),
    )


def resolve_run_root(source_path: str | Path) -> Path:
...
def parse_import_spec(raw_value: object) -> ImportSpec:
    """Parse one `import_from` item from config.
    ...
    - `{"path": "/path/to/old/run", "import_optimizers": false}`
    - `{"path": "/path/to/old/run", "solver_ids": [...], "optimizer_ids": [...]}`

    The returned `ImportSpec` is the normalized internal representation used by
    the runner/factory import flow. When `solver_ids` or `optimizer_ids` is
    omitted, that side imports the full source population.
    """

    if isinstance(raw_value, str) and raw_value:
        return ImportSpec(path=Path(raw_value))
    if not isinstance(raw_value, dict):
        raise ValueError("Each `import_from` entry must be a path string or a mapping.")
    path = raw_value.get("path")
    if not isinstance(path, str) or not path:
        raise ValueError("Each import mapping must contain a non-empty `path`.")
    unknown_keys = set(raw_value) - _IMPORT_SPEC_KEYS
    if unknown_keys:
        keys = ", ".join(sorted(unknown_keys))
        allowed = ", ".join(sorted(_IMPORT_SPEC_KEYS))
        raise ValueError(f"Unknown import mapping key(s): {keys}. Allowed keys: {allowed}.")
    solver_ids = _parse_import_ids(raw_value.get("solver_ids"), key="solver_ids")
    optimizer_ids = _parse_import_ids(raw_value.get("optimizer_ids"), key="optimizer_ids")
    return ImportSpec(
        path=Path(path),
        solver_ids=solver_ids,
        optimizer_ids=optimizer_ids,
        import_solvers=_parse_import_bool(raw_value.get("import_solvers"), key="import_solvers"),
        import_optimizers=_parse_import_bool(
            raw_value.get("import_optimizers"), key="import_optimizers"
        ),
    )


def _parse_import_ids(raw_value: object, *, key: str) -> tuple[str, ...]:
...
def _parse_import_bool(raw_value: object, *, key: str) -> bool:
    if raw_value is None:
        return True
