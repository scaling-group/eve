...
def test_import_populations_from_previous_run_rewrites_artifacts_into_target_run(
    tmp_path: Path,
) -> None:
    ...
    try:
        report = import_populations_from_run(
        ...
        assert report.solver.entries_imported == 1
        assert report.optimizer.entries_imported == 1

        imported_solver = target_solver_population.entries()
        imported_optimizer = target_optimizer_population.entries()
        assert [entry.id for entry in imported_solver] == ["solver_old"]
        assert [entry.id for entry in imported_optimizer] == ["optimizer_old"]
...
def test_resolve_run_root_accepts_nested_workspace_path(tmp_path: Path) -> None:
    ...
    resolved = resolve_run_root(nested)

    assert resolved == run_root


def test_import_populations_from_multiple_sources_is_additive(tmp_path: Path) -> None:
    target_root = tmp_path / "target_run"
    target_root.mkdir()
    target_solver_store = SQLiteLineageStore(target_root / "solver_lineage.db")
    target_optimizer_store = SQLiteLineageStore(target_root / "optimizer_lineage.db")
...
def test_import_populations_can_filter_specific_solver_and_optimizer_ids(tmp_path: Path) -> None:
    source_root = tmp_path / "source_run"
    source_root.mkdir()
    source_solver_store = SQLiteLineageStore(source_root / "solver_lineage.db")
    source_optimizer_store = SQLiteLineageStore(source_root / "optimizer_lineage.db")
...
def test_import_populations_can_skip_optimizer_side(tmp_path: Path) -> None:
    source_root = tmp_path / "source_run"
    source_root.mkdir()
    source_solver_store = SQLiteLineageStore(source_root / "solver_lineage.db")
    source_optimizer_store = SQLiteLineageStore(source_root / "optimizer_lineage.db")
    ...
    SolverPopulation(
        ...
        run_id="source_solver",
        config=_config(source_root),
    ).add(PopulationEntry(id="solver_old", files={"a.py": "a\n"}, score=_score(1.0), logs={}))
    OptimizerPopulation(
        source_optimizer_store,
        source_artifact_store,
        run_id="source_optimizer",
        config=_config(source_root),
    ).add(PopulationEntry(id="optimizer_old", files={"x.md": "x\n"}, score=_score(1500.0), logs={}))
    ...
    try:
        report = import_populations_from_run(
            ...
            solver_population=target_solver_population,
            optimizer_population=target_optimizer_population,
            import_optimizers=False,
        )

        assert report.solver.entries_imported == 1
        assert report.optimizer.entries_imported == 0
        assert {entry.id for entry in target_solver_population.entries()} == {"solver_old"}
        assert target_optimizer_population.entries() == []
    finally:
        source_solver_store.close()
        source_optimizer_store.close()
        target_solver_store.close()
...
def test_parse_import_spec_accepts_filtered_mapping() -> None:
    spec = parse_import_spec(
        {
            "path": "/tmp/demo-run",
    ...
    assert spec.import_solvers is True
    assert spec.import_optimizers is False


def test_parse_import_spec_rejects_unknown_keys() -> None:
    with pytest.raises(ValueError, match="Unknown import mapping key"):
        parse_import_spec(
            {
                "path": "/tmp/demo-run",
                "unexpected_toggle": False,
            }
        )
